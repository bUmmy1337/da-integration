...
public class SliderComponent extends AbstractSettingComponent {
    ...
    public SliderComponent(String name, String description, float defaultValue, float min, float max,
                           boolean isInteger, java.util.function.Supplier<Float> getter,
        ...
        if (range > 0) {
            this.animatedPercentage = (getter.get() - minValue) / range;
        }
    }
    ...
    public void render(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        float range = maxValue - minValue;
        float targetPercentage = range > 0 ? (getter.get() - minValue) / range : 0f;
        ...
        RenderHelper.text(g, settingName, x + 9.5f, y + 1f, 6,
                Theme.withAlpha(Theme.TEXT_PRIMARY, (int)(200 * alphaMultiplier)));

        renderValueInput(g, font, mouseX, mouseY);
        renderSlider(g);
    }
    ...
    private void renderValueInput(GuiGraphicsExtractor g, net.minecraft.client.gui.Font font,
                                  int mouseX, int mouseY) {
        float currentValue = getter.get();
        String valueText = isInteger
                ? String.valueOf((int) currentValue)
                : String.format("%.1f", currentValue);
        String unitsText = " units";
        String fullText = valueText + unitsText;

        float fullTextWidth = FontHelper.width(font, fullText);
        float valueTextWidth = FontHelper.width(font, valueText);

        float baseX = x + width - fullTextWidth - 4;
        float textY = y + 2f;

        float centerOffset = (FontHelper.width(font, unitsText) / 2f) * valueOffsetX;
        ...
        if (backgroundAlpha > 0.01f) {
            ...
            RenderHelper.rect(g, inputBoxX, inputBoxY, inputBoxWidth, inputBoxHeight,
        ...
        if (combinedOutlineAlpha > 0.01f) {
            ...
            RenderHelper.outline(g, inputBoxX, inputBoxY, inputBoxWidth, inputBoxHeight,
                    0.1f, Theme.withAlpha(Theme.BORDER, outlineAlpha));
        }

        if (inputMode && inputAnimation > 0.5f) {
            String displayText = inputText;
            float displayTextWidth = FontHelper.width(font, displayText);
            ...
            RenderHelper.text(g, displayText, centeredX, textY, 5,
                    Theme.withAlpha(Theme.TEXT_PRIMARY, textAlpha));

            long currentTime = System.currentTimeMillis();
            if ((currentTime % 1000) < 500) {
                String beforeCursor = inputText.substring(0, cursorPosition);
                float cursorX = centeredX + FontHelper.width(font, beforeCursor);
                int cursorAlpha = clampAlpha(255 * inputAnimation * alphaMultiplier);
                RenderHelper.rect(g, cursorX, inputBoxY + 2, 0.5f, inputBoxHeight - 4,
                        Theme.withAlpha(Theme.TEXT_PRIMARY, cursorAlpha));
            }
        } else {
            ...
            if (valueAlphaInt > 0) {
                RenderHelper.text(g, valueText, currentValueX, textY, 5,
                        Theme.withAlpha(Theme.TEXT_MUTED, valueAlphaInt));
            }

            if (unitsAlpha > 0.01f) {
                int unitsAlphaInt = clampAlpha(160 * unitsAlpha * alphaMultiplier);
                if (unitsAlphaInt > 0) {
                    RenderHelper.text(g, unitsText, currentValueX + valueTextWidth, textY, 5,
                            Theme.withAlpha(Theme.TEXT_MUTED, unitsAlphaInt));
                }
            }
        }
    }

    private void renderSlider(GuiGraphicsExtractor g) {
        float sliderY = y + 11;
        float sliderHeight = 2.5f;
        float sliderPadding = 1f;
        float sliderTrackWidth = width - 2;

        RenderHelper.rect(g, x + sliderPadding, sliderY, sliderTrackWidth, sliderHeight,
                Theme.withAlpha(Theme.BG_INPUT, (int)(220 * alphaMultiplier)));

        float filledWidth = sliderTrackWidth * animatedPercentage;

        if (filledWidth > 0) {
            RenderHelper.rect(g, x + sliderPadding, sliderY, filledWidth, sliderHeight,
                    Theme.withAlpha(Theme.TEXT_MUTED, (int)(230 * alphaMultiplier)));
        }
        ...
        RenderHelper.rect(g, knobX, knobY, knobSize, knobSize,
                Theme.withAlpha(Theme.TEXT_PRIMARY, (int)(255 * alphaMultiplier)));
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            if (isValueHover(mouseX, mouseY) && !inputMode) {
                inputMode = true;
                float currentValue = getter.get();
                String currentValueStr = isInteger
                        ? String.valueOf((int) currentValue)
                        : String.format("%.1f", currentValue);
                inputText = currentValueStr;
    ...
    private void updateValue(double mouseX) {
        ...
        float range = maxValue - minValue;
        float newValue = minValue + (range * percentage);
